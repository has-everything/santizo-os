...
- `js/os.js` — boots the right mode (`matchMedia ≤820px` → mobile, else desktop; crossing the breakpoint reloads), scales the page up on screens much wider than the ~1730px design (`body zoom`, capped 1.6x, height-aware; drag math divides pointer deltas by it), and runs the desktop window manager: drag, focus/z-order, minimize (roll up), maximize (1.6x width, or full-page for `maxFull` media windows: reel, photos, projects), drag-to-trash with put-back, photo gallery, menu clock, initial layout. Rarely needs editing.
